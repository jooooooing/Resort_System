...
String query1 = "select count(*) from resort_review;";
ResultSet rset1 = stmt.executeQuery(query1);		// 총 레코드 수를 조회하는 쿼리문 실행하여 결과를 ResetSet 객체에 저장
rset1.next();
count_total = rset1.getInt(1);

stmt = conn.createStatement();
String query2 = "select * from resort_review order by rootid desc, recnt asc limit " + ((Integer.parseInt(no_page)-1)*size_page) + ", " + size_page + ";";
...
<jsp:include page="menu.jsp" />
	...
	<div class="container">
     <table class="table-view">
      <tr class="title">
        <td width="50px" align=center><b>번호</b></td>
        <td width="300px" align=center><b>제목</b></td>
        <td width="100px" align=center><b>조회수</b></td>
        <td width="100px" align=center><b>등록일</b></td>
      </tr>
      <%
        while(rset2.next()) {
          out.println("<tr>");											
          out.println("<td align=center>" + rset2.getInt("id") + "</td>");
          out.print("<td align=left><a href='e_02_review_oneView.jsp?key=" + rset2.getString("id") + "'>");
          ...
          out.print("</a></td>");
          out.println("<td align=center>" + rset2.getInt("viewcnt") + "</td>");
          out.println("<td align=center>" + rset2.getDate("date") + "</td>");
          out.println("</tr>"); 
        }
      %>
    </table>
	...
	<table style="margin-left:auto; margin-right:auto;">
		<tr>
			<td width=600></td>
			<td>
				<input type=submit OnClick="window.location.href='ReNotice_insert.jsp'" value=신규></input>
			</td>
		</tr>
	</table>
...
<div class="pagination">
      <%
        ...
        if ( no_page_start != 1) {					// 첫 페이징 그룹이 아닐 때만 <와 << 표시
          out.println("<tr><td><a href='e_02_review.jsp'>&lt;&lt;</a></td>");			// << 출력하고 첫 페이지로 이동 링크
          out.println("<td><a href='e_02_review.jsp?page=" + (no_page_start-1) + "'>&lt;</a></td>"); // < 출력하고 이전 페이징 그룹의 마지막 페이지 링크
        }
        ...
        for (int i = no_page_start ; i < no_page_start + size_group_page ; i++) {	// 현재 페이지 번호가 속한 페이징의 그룹의 (첫 페이지 번호) 부터 (첫 페이지 번호+페이징 그룹의 크기) 까지 반복
          ...
          if (i == no_page_now) {					// 현재에 해당하는 페이지 번호이면 굵은 글씨로 표시
            out.println("<td><b><a href='e_02_review.jsp?page=" + i + "'>" + i + "</a></b></td>");	// 현재 페이징 그룹에 해당하는 페이지 번호 출력
          } else {								// 아니면 일반 굵기로 표시
            out.println("<td><a href='e_02_review.jsp?page=" + i + "'>" + i + "</a></td>");	// 현재 페이징 그룹에 해당하는 페이지 번호 출력
          }
        }
        ...
        if (!(no_page_start <= no_page_last && no_page_last <= no_page_start + size_group_page) && (no_page_last!=0)) {	// 마지막 페이징 그룹이 아닐 때와 출력할 데이터가 있을 때만 >와 >> 표시
          out.println("<td><a href='e_02_review.jsp?page=" + (no_page_start + size_group_page) + "'>&gt;</a></td>"); // > 출력하고 다음 페이징 그룹의 첫 페이지 링크
          out.println("<td><a href='e_02_review.jsp?page=" + no_page_last + "'>&gt;&gt;</a></td>");				// >> 출력하고 마지막 페이지로 이동 링크
        }
